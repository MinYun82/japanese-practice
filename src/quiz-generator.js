...
var QuizGenerator = (function () {
  ...
  // ========================================================================
  var GOJUON = [
    ['あ', 'ア', 'a', 'ㄚ'],   ['い', 'イ', 'i', 'ㄧ'],   ['う', 'ウ', 'u', 'ㄨ'],   ['え', 'エ', 'e', 'ㄝ'],   ['お', 'オ', 'o', 'ㄛ'],
    ['か', 'カ', 'ka', 'ㄎㄚ'], ['き', 'キ', 'ki', 'ㄎㄧ'], ['く', 'ク', 'ku', 'ㄎㄨ'], ['け', 'ケ', 'ke', 'ㄎㄝ'], ['こ', 'コ', 'ko', 'ㄎㄛ'],
    ['さ', 'サ', 'sa', 'ㄙㄚ'], ['し', 'シ', 'shi', 'ㄒ'],  ['す', 'ス', 'su', 'ㄙㄨ'], ['せ', 'セ', 'se', 'ㄙㄝ'], ['そ', 'ソ', 'so', 'ㄙㄛ'],
    ['た', 'タ', 'ta', 'ㄊㄚ'], ['ち', 'チ', 'chi', 'ㄑ'],  ['つ', 'ツ', 'tsu', 'ㄗㄨ'],['て', 'テ', 'te', 'ㄊㄝ'], ['と', 'ト', 'to', 'ㄊㄛ'],
    ['な', 'ナ', 'na', 'ㄋㄚ'], ['に', 'ニ', 'ni', 'ㄋㄧ'], ['ぬ', 'ヌ', 'nu', 'ㄋㄨ'], ['ね', 'ネ', 'ne', 'ㄋㄝ'], ['の', 'ノ', 'no', 'ㄋㄛ'],
    ['は', 'ハ', 'ha', 'ㄏㄚ'], ['ひ', 'ヒ', 'hi', 'ㄏㄧ'], ['ふ', 'フ', 'fu', 'ㄈㄨ'], ['へ', 'ヘ', 'he', 'ㄏㄝ'], ['ほ', 'ホ', 'ho', 'ㄏㄛ'],
    ['ま', 'マ', 'ma', 'ㄇㄚ'], ['み', 'ミ', 'mi', 'ㄇㄧ'], ['む', 'ム', 'mu', 'ㄇㄨ'], ['め', 'メ', 'me', 'ㄇㄝ'], ['も', 'モ', 'mo', 'ㄇㄛ'],
    ['や', 'ヤ', 'ya', 'ㄧㄚ'], ['ゆ', 'ユ', 'yu', 'ㄧㄨ'], ['よ', 'ヨ', 'yo', 'ㄧㄛ'],
    ['ら', 'ラ', 'ra', 'ㄌㄚ'], ['り', 'リ', 'ri', 'ㄌㄧ'], ['る', 'ル', 'ru', 'ㄌㄨ'], ['れ', 'レ', 're', 'ㄌㄝ'], ['ろ', 'ロ', 'ro', 'ㄌㄛ'],
    ['わ', 'ワ', 'wa', 'ㄨㄚ'], ['を', null, 'wo', 'ㄛ'],   ['ん', 'ン', 'n', 'ㄣ']
  ];

  // 展開成卡片池（平假名全收；片假名跳過幾乎用不到的 ヲ）
  var KANA_GAME_POOL = [];
  GOJUON.forEach(function (row) {
    KANA_GAME_POOL.push({ char: row[0], reading: row[2], zhuyinHint: row[3], kana: 'hiragana' });
    if (row[1]) {
      KANA_GAME_POOL.push({ char: row[1], reading: row[2], zhuyinHint: row[3], kana: 'katakana' });
    }
  });
  ...
  // ========================================================================
  // 假名閃卡遊戲 — 隨機抽 N 張，看字猜發音，點卡翻面對答案
  // 與「翻卡認識」不同：不綁定週進度、每次純隨機、混合平假名+片假名
  // ========================================================================
  function generateKanaGame(options) {
    var opts = options || {};
    var count = opts.count || 10;
    return shuffle(KANA_GAME_POOL).slice(0, count).map(function (k) {
      return {
        char: k.char,
        reading: k.reading,
        zhuyinHint: k.zhuyinHint,
        kana: k.kana,
        audioText: k.char
      };
    });
